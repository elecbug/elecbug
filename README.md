# Profile

**Computer Network / Distributed Systems Engineer**  
**Building large-scale P2P testbeds, blockchain experiments, and research infrastructure**

## Bio

I am a graduate student and engineer working on P2P networks, blockchain systems,
and large-scale distributed experimentation.
My interests include GossipSub/libp2p analysis, network graph metrics,
containerized testbed design, and research infrastructure automation.

- **M.S. Course**, Dept. Computer Engineering, *[Computer Network Lab](https://comnet.kmu.ac.kr), [Keimyung University](https://www.kmu.ac.kr)*, Korea, Present.  
- **B.S.**, Dept. Computer Engineering, *[Keimyung University](https://www.kmu.ac.kr)*, Korea, Feb. 2025.

## Tech Stack

<details>
<summary>Show</summary>

### Research & Domain

* P2P Networks
* Distributed Systems
* Blockchain Systems
* GossipSub / libp2p
* Network Graph Analysis
* Large-scale P2P Testbed Design
* Distributed Network Experimentation

### Programming Languages

* C / C++
* C#
* Go
* Rust
* Python
* Java
* JavaScript

### Libraries & Frameworks

* libp2p
* OpenSSL
* .NET
* Node.js
* NetworkX
* NumPy / SciPy

### Infrastructure & Systems

* Research Lab Infrastructure Design
* On-premise Server Operations
* Containerized Experiment Platforms
* Docker / Docker Swarm
* PXE-based System Deployment
* Linux Server Administration

### Storage & Data Reliability

* RAID Storage Architecture
* NAS Storage Management
* Data Integrity & Backup Strategy
* Storage Capacity Planning

### Networking & Monitoring

* Linux Networking
* Network Diagnostics
* Wireshark
* Prometheus
* Grafana

### Data & Experimentation

* Python Data Analysis
* Graph-based Metric Analysis
* Network Experiment Automation

### System & Database

* Ubuntu / Linux
* Windows
* MySQL

### Development & Toolchain

* Git
* GCC / Clang

### Documentation

* Markdown
* LaTeX
* HTML / CSS

</details>

## Certification

<details>
<summary>Show (in Korea)</summary>

- Craftsman Electricity, HRDK, Korea, Dec. 12, 2018.
- Craftsman Elevator, HRDK, Korea, Jun. 21, 2019.
- Information·Computer Secondary School Regular Teacher (Level 2), Ministry of Education, Korea, Feb. 18, 2025.
- Engineer Information Processing, HRDK, Korea, Sep. 12, 2025.

</details>

## Conference & Journal

<details>
<summary>Show (Korean Paper)</summary>

- **이성욱**, 주홍택, **"K-P2PLab: P2P 네트워크 토폴로지 분석을 위한 테스트베드 및 분석 플랫폼 개발,"** in *KNOM Review,* vol. 27, no. 2, pp. 40-48, Dec. 2024.
  - [doi:10.22670/knom.2024.27.2.40](https://doi.org/10.22670/knom.2024.27.2.40).
- **이성욱**, 김형엽, 김승민, 주홍택, **"GossipSub Protocol 기반 P2P 네트워크에서 통신 지연시간이 브로드캐스트 효율성에 미치는 영향분석,"** in *Proc. 2025 통신망운용관리 학술대회,* Daejeon, Korea, pp. 12-16, Apr. 2025.
  - [Online](http://dpnm.postech.ac.kr/papers/KNOM/25/KNOM2025_proceedings.pdf).

</details>

<details>
<summary>Show (English Paper)</summary>

- **Sungwook Lee**, Hyungyeop Kim, Seungmin Kim and Hongtaek Ju, **"An Analysis of the Impact of Network Delay and Churn on Broadcast Efficiency in GossipSub-based P2P Networks,"** in *Proc. 2025 25th Asia-Pacific Network Operations and Management Symposium (APNOMS),* Kaohsiung, Taiwan, pp. 1-4, Sep. 2025.
  - [doi: 10.23919/APNOMS67058.2025.11181302](https://doi.org/10.23919/APNOMS67058.2025.11181302).
- **Sungwook Lee**, Hyungyeop Kim, Seungmin Kim and Hongtaek Ju, **"K-P2PLab: A Scalable Docker Swarm-Based Testbed for Peer-to-Peer Topology Analysis,"** in *Proc. 2025 25th Asia-Pacific Network Operations and Management Symposium (APNOMS),* Kaohsiung, Taiwan, pp. 1-4, Sep. 2025.
  - [doi: 10.23919/APNOMS67058.2025.11181317](https://doi.org/10.23919/APNOMS67058.2025.11181317).

</details>

## Community Contribution

<details>
<summary>Show</summary>

- **Docker Korea Translation Project**
  - Maintainer of [Docker Korea](https://github.com/docker-ko/docker-ko.github.io), translating Docker official documentation into Korean to support local developers.
- **KMU Developer Group Save**
  - Organizer of the Keimyung University Computer S&E academic community DUTO.
  - Maintainer of [KMU Developer Group Save](https://github.com/kmu-save), online study group for development at Keimyung University.
- **Contributor for libp2p**
  - Contributor to the [libp2p project](https://github.com/libp2p).

</details>

## Award

<details>
<summary>Show (in Keimyung University)</summary>

- 창의 IT 경진대회, 프로그램 해커톤 - 우수상, 계명대학교, Oct. 22, 2022.
- 국가우수장학금(이공계), 과학기술정보통신부, Oct. 31, 2023
- 인공지능 소프트웨어 경진대회, AI 프로그램 - 대상, 계명대학교, Sep. 09, 2024
- 창립 125주년 기념 대학생 에세이 대회, 자연공학분야 - 우수상, 계명대학교, Dec. 06, 2024.

</details>
